...
CACHE_TTL_TRENDING = 3600  # 1 hour for trending
CACHE_TTL_CATEGORY = 3600  # 1 hour for category
CACHE_TTL_DETAIL = 7200  # 2 hours for book details
CACHE_TTL_RECOMMENDATIONS = 1800  # 30 minutes for recommendations

async def unified_search(query: str, limit: int = 12, page: int = 1) -> dict:
    """
    Parallel search across multiple providers with Redis caching.
    """
    # Generate cache key
    cache_key = f"book:search:{query.lower()}:{limit}:{page}"
    ...
    cache_set(cache_key, result, ttl=CACHE_TTL_SEARCH)
    logger.info(f"Cache SET: {cache_key} (TTL: {CACHE_TTL_SEARCH}s)")
...
async def unified_books_by_category(category: str, limit: int = 12) -> dict:
    ...
    if cat_lower == "all":
        ...
        cache_set(cache_key, result, ttl=CACHE_TTL_CATEGORY)
        return result
